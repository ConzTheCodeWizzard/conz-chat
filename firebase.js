...
banner.style.cssText = position:fixed;top:0;left:0;right:0;z-index:999999; background:#1a1a1a;border-bottom:2px solid #ff0033; padding:12px 16px;display:flex;align-items:center;gap:12px; box-shadow:0 4px 20px rgba(255,0,51,0.4); animation:slideDown 0.3s ease; cursor:pointer;;
banner.innerHTML = ${icon ? <img src="${icon}" style="width:36px;height:36px;border-radius:50%;">: ""} &lt;div style="flex:1;"&gt; &lt;div style="font-weight:bold;font-size:14px;color:#fff;"&gt;${title}</div>
<div style="font-size:13px;opacity:0.75;color:#ccc;margin-top:2px;">${body}&lt;/div&gt; &lt;/div&gt; &lt;div style="font-size:18px;opacity:0.5;padding:4px 8px;"&gt;✕&lt;/div&gt;;
