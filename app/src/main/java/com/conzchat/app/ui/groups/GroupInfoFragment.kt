...
class GroupInfoFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        FirebaseManager.groupsRef.document(groupId).get().addOnSuccessListener { doc ->
        ...
        binding.btnLeave.setOnClickListener {
            FirebaseManager.groupsRef.document(groupId)
        ...
        binding.btnAddMember.setOnClickListener {
            val username = binding.etAddMember.text.toString().trim()
            if (username.isEmpty()) return@setOnClickListener
            FirebaseManager.usersRef.whereEqualTo("usernameLower", username.lowercase()).get()
                .addOnSuccessListener { snap ->
                    val newUid = snap.documents.firstOrNull()?.id ?: run {
                        context?.toast("User not found"); return@addOnSuccessListener
                    }
                    FirebaseManager.groupsRef.document(groupId)
                        .update("members", com.google.firebase.firestore.FieldValue.arrayUnion(newUid))
                        .addOnSuccessListener {
                            context?.toast("Member added!")
                            binding.etAddMember.setText("")
                        }
                }
        }
    }
